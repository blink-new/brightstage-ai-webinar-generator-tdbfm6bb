...
export function ContentInputStep({ data, onUpdate, onNext, onPrev }: ContentInputStepProps) {
  ...
  // Check if blink client is ready
  useEffect(() => {
    const checkBlinkReady = async () => {
      try {
        // Wait a moment for blink to initialize
        await new Promise(resolve => setTimeout(resolve, 1000))
        if (blink && blink.ai) {
          setIsInitializing(false)
        } else {
          // Keep checking every second for up to 10 seconds
          let attempts = 0
          const checkInterval = setInterval(() => {
            ...
            if (blink && blink.ai) {
              ...
              toast({
                ...
                description: 'AI features may be limited. Please refresh if issues persist.',
                variant: 'destructive'
              })
            }
          }, 1000)
        ...
        console.error('Error checking blink readiness:', error)
        setIsInitializing(false)
      }
    }
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            <Button
              ...
              disabled={isEnhancing || !data.description.trim() || isInitializing}
            >
              {isEnhancing ? (
              ...
              {isInitializing ? 'Initializing...' : 'Enhance with AI'}
            </Button>
          </div>
        </div>
      ...
      <div className="flex justify-between pt-6 border-t">
        ...
        <Button
          ...
          disabled={!canProceed || isGenerating || isInitializing}
          size="lg"
