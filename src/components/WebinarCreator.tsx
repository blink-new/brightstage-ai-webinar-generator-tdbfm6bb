...
import { WebinarDataConverter } from '../utils/webinarDataConverter'
...
function WebinarCreator({ user, onBack, editingWebinarId }: WebinarCreatorProps) {
  ...
  useEffect(() => {
    if (editingWebinarId) {
      try {
        ...
        if (existingWebinar) {
          const safeWebinarData = {
            ...
            exportUrls: existingWebinar.exportUrls
          }
          
          setWebinarData(safeWebinarData)
          ...
          if (existingWebinar.videoUrl) {
            ...
            setCurrentStep(3)
          } else if (existingWebinar.slides && existingWebinar.slides.length > 0) {
            setCurrentStep(2)
          } else {
            setCurrentStep(1)
          }
        ...
        console.error('Error loading existing webinar:', error)
      ...
      setCurrentStep(1)
      setCurrentWebinarId(undefined)
    }
  }, [editingWebinarId, getProject])

  const handleSave = useCallback(async (silent = false) => {
    try {
      // Ensure title is set - fix potential undefined access
      ...
      const projectData = WebinarDataConverter.toProject(dataToSave, user.id)
      
      if (currentWebinarId) {
        // Update existing project
        await updateProject(currentWebinarId, projectData)
      } else {
        // Create new project
  ...
  }, [webinarData, user.id, currentWebinarId, createProject, updateProject, toast])
  ...
  const updateWebinarData = (updates: Partial<WebinarData>) => {
    setWebinarData(prev => WebinarDataConverter.merge(prev, updates))
  }
